...
% BZ = brillouin_zone('sc',path,ones(3,1),num);
%
% if strcmp(dim(1:2),'1d')
%     sz = length(BZ.k(1,:));
%     k = BZ.k;
%     k(1,:) = k(1,:)/2;
%     k(2,:) = k(2,:)*sqrt(3)/2;
%     w = ones(size(k));
% else
%     [kmp,idxvec] = monkhorst_pack_grid_hex([num num 1],'shift');
%     sz = size(kmp,2);
%     k(1,:) = kmp(1,:)*pi/2;
%     k(2,:) = kmp(2,:)*pi/2*sqrt(3);
%     k(3,:) = zeros(sz,1);
% end
...
Ek = zeros(6,sz);
coeff = complex(zeros(6,6,sz,6));
gradA_H = complex(zeros(6,6,sz,6));
gradB_H = complex(zeros(6,6,sz,6));
...
for tri_idx = 1:6
    ...
    for ii = 1:sz
        ...
        ktmp = [k(ii,1,tri_idx) k(ii,2,tri_idx) k(ii,3,tri_idx)];
        A = ktmp(1) * pi / 2;
        B = ktmp(2) * pi / 2 * sqrt(3);
        C = ktmp(3);
        ...
        Ek(1:3,ii) = Ek1(:);
        Ek(4:6,ii) = Ek2(:);
        coeff(1:3,1:3,ii,tri_idx) = V1;
        coeff(4:6,4:6,ii,tri_idx) = V2;
        ...
        % Speichere H
        % M(ii).H = H;
        % Überprüfe Hermitizität
        % check(ii) = sum(sum(1iag(conj(H)-H')>=tol + real(conj(H)-H')>=tol));
        ...
        gradA_H(1:3,1:3,ii,tri_idx) = dA_H; % * a / 2
        gradA_H(4:6,4:6,ii,tri_idx) = dA_H; % * a / 2
        
        gradB_H(1:3,1:3,ii,tri_idx) = dB_H; % * sqrt(3) / 2 * a
        gradB_H(4:6,4:6,ii,tri_idx) = dB_H; % * sqrt(3) / 2 * a
    end
